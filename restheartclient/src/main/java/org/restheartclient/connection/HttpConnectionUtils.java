...
public class HttpConnectionUtils implements IHttpConnectionsUtil, Closeable {
	 ...
	 * UsernamePasswordCredentials("user1", "user1Pass");
	 ...
	 * HttpGet(URL_SECURED_BY_BASIC_AUTHENTICATION)); int statusCode =
	...
	public <REQ> CloseableHttpResponse sendHttpPost(String url, REQ request) {
		...
		try {
			...
			LOGGER.log(Level.SEVERE,
					"was unable to send POST request:" + requestJson + " (displaying first 1000 chars) from url-" + url,
					e);
		}
	...
	//	String requestJson = GsonUtils.toJson(request);
		...
		try {
			...
			LOGGER.log(Level.SEVERE,
					"Was unable to send PUT request:" + requestJson + " (displaying first 1000 chars) from url-" + url,
					e);
		}
